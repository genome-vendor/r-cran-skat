...
, BetaType = c("Log","Fixed"), MaxBeta=1.6, Negative.Percent=0)
...
\arguments{
      ...
      \item{BetaType}{a function type of effect sizes (default= ``Log''). ``Log'' indicates that effect size of each causal variant equals to \eqn{c|log10(MAF)|}, and ``Fixed'' indicates that effect sizes of all causal variants are the same.}
      \item{MaxBeta}{a numeric value of the maximum effect size (default= 1.6). When BetaType="Log", the maximum effect size is MaxBeta (when MAF=0.0001). When BetaType="Fixed", all causal variants have the same effect size (= MaxBeta). See details}
      \item{Negative.Percent}{a numeric value of the percentage of coefficients of causal variants that are negative (default= 0).}
      \item{r.corr}{(Power_Continuous_R only) the \eqn{\rho} parameter of new class of kernels with compound symmetric correlation structure for genotype effects  (default= 0). See details.}
}
...
When BetaType=``Log'', MaxBeta is the coeffecient value (\eqn{\beta}) of the causal SNP with MAF \eqn{= 10^{-4}} 
and used to obtain c value of the function \eqn{c|log10(MAF)|}. For example, if MaxBeta=1.6, \eqn{ c = 1.6/4 = 0.4}. Then a variant with MAF=0.001 has \eqn{\beta = 1.2} and a variant with MAF=0.01 has \eqn{\beta = 0.8}.

When the SubRegion.Length is small such as 3kb or 5kb, it is possible that you can have different estimated power for each run with N.Sim = \eqn{50 \sim 100}. Then, please increase the N.Sim to \eqn{500 \sim 1000} to obtain stable results. 
